...
TF_CFLAGS := $(shell /usr/bin/env python -c 'import tensorflow as tf; print(" ".join(tf.sysconfig.get_compile_flags()))')
TF_LFLAGS := $(shell /usr/bin/env python -c 'import tensorflow as tf; print(" ".join(tf.sysconfig.get_link_flags()))')
